...
import { AdmOccurrences, BarracksChanges } from '@/types';
...
interface UsersFormProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: AdmOccurrences;
  isEditing?: boolean;
  id_bookday: string
}

export function Step3AdmOccurrences({ id_bookday, isOpen, onClose, initialData, isEditing = false }: UsersFormProps) {
  ...
  const onSubmit = async (data: z.infer<typeof peopleSchema>) => {
    ...
    try {
      ...
      if (isEditing && initialData?._id) {
        console.log('SALVOU');
        await myApi.put(`/api/administrativeOccurrences/${initialData._id}`, payload);
        toast.success('Ocorrencia administrativa atualizada com sucesso!');
      } else {
        await myApi.post('/api/administrativeOccurrences/', payload);
        toast.success('Ocorrencia administrativa criada com sucesso!');
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Editar Dados da carga das ocorrencias administrativas' : 'Cadastro das ocorrencias administrativas'}</DialogTitle>
          <DialogDescription>
            {isEditing ? 'Edite as informações das ocorrencias administrativas.' : 'Adicione uma nova ocorrencias administrativas.'}
          </DialogDescription>
        </DialogHeader>
