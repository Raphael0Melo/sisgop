...
export function Step5Gather({ id_bookday, isOpen, onClose, initialData, isEditing = false }: UsersFormProps) {
  ...
  const onSubmit = async (data: z.infer<typeof peopleSchema>) => {
    ...
    try {
      ...
      if (isEditing && initialData?._id) {
        console.log('SALVOU');
        await myApi.put(`/api/gather/${initialData._id}`, payload);
        toast.success('Juntada atualizada com sucesso!');
      } else {
        await myApi.post('/api/gather/', payload);
        toast.success('Juntada criada com sucesso!');
      }

      onClose();
    } catch (error) {
      console.error('Erro ao salvar permissão:', error);
      toast.error('Erro ao salvar juntada.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Editar Dados da juntada' : 'Cadastro das juntadas'}</DialogTitle>
          <DialogDescription>
            {isEditing ? 'Edite as informações da juntada.' : 'Adicione uma nova juntada.'}
          </DialogDescription>
        </DialogHeader>
