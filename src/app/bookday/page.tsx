...
export default function BookDay() {
  const [data, setData] = useState<BookDaysAll[]>([]);
  ...
  const [editingEntry, setEditingEntry] = useState<BookDaysAll | null>(null);
  const [loading, setLoading] = useState(false);

  async function handleFetchData() {
    setLoading(true);
    try {
      const response = await myApi.get('/api/bookDay/1/20');
      const data = response.data.result;
      setBookCurrent(response.data.responseBookToday);
  ...
  const filteredEntries = data.filter(entry =>
    entry.numero_parte.toLowerCase().includes(searchTerm.toLowerCase()) ||
    entry.data_inicial.toLowerCase().includes(searchTerm.toLowerCase())
  );


  const handleEdit = (entry: BookDaysAll) => {
    setEditingEntry(entry);
    setIsFormOpenEdit(true);
  };

  const handleVisualize = (entry: BookDaysAll) => {
    PdfBookDay(entry);
  };
        ...
        <MainLayout>
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-white">Livro do Dia</h1>
                <p className="text-white">Registro de ocorrências e atividades diárias</p>
              </div>
              <Button
            ...
            <Card>
              ...
              <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Nome do relator</TableHead>
                  ...
                  <TableBody>
                    {filteredEntries.length === 0 ? (
                      ...
                      filteredEntries.map((entry: BookDaysAll) => (
                        <TableRow key={entry._id}>
                          <TableCell>{entry.nome_completo_relator}</TableCell>
                          <TableCell className="max-w-md truncate">{entry.turno}</TableCell>
                          <TableCell className="max-w-md truncate">{entry.tipo_servico}</TableCell>
                          <TableCell>
