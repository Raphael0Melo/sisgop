...
export default function Situation() {
  ...
  const [editingEntry, setEditingEntry] = useState<Situations | null>(null);
  const [loading, setLoading] = useState(false);

  async function handleFetchData() {
    setLoading(true);
    try {
      const response = await myApi.get('/api/situation');
  ...
  const filteredEntries = data.filter(entry =>
    entry.name_situation.toLowerCase().includes(searchTerm.toLowerCase()) ||
    entry.group.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleEdit = (entry: Situations) => {
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white">Gerenciar Situações</h1>
            <p className="text-white">Registro de Situações</p>
          </div>
          <Button
        ...
        <Card>
          <CardHeader>
            <CardTitle>Registros de Situações</CardTitle>
            <CardDescription>
                ...
                {filteredEntries.length === 0 ? (
                  ...
                  filteredEntries.map((entry) => (
                    <TableRow key={entry._id}>
                      <TableCell className="max-w-md truncate">{entry.name_situation}</TableCell>
